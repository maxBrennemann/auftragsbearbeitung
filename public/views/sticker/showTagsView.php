<dl class="tagList block bg-gray-50 rounded-lg p-2" id="tagList">
    <?php foreach ($tags as $tag):?>
        <dt class="cursor-default inline-flex rounded-lg font-semibold overflow-hidden" data-binding="true" data-fun="manageTag">
    ...
    <?php else: ?>
        <?php foreach ($suggestionTags as $tag): ?>
            <dt class="suggestionTag inline-flex bg-gray-400 rounded-lg font-semibold px-2 py-1 cursor-pointer text-slate-700 hover:text-slate-950 hover:bg-gray-300" data-binding="true" data-fun="manageTag">
                <span><?=$tag?></span>
            </dt>
        <?php endforeach; ?>
    <?php endif; ?>
