...
class Aufgabenliste
{
    ...
    {
        $query = "SELECT IF(kunde.Firmenname = '', CONCAT(kunde.Vorname, ' ', kunde.Nachname), kunde.Firmenname) as Name, auftrag.Auftragsbezeichnung, schritte.Bezeichnung, IF(schritte.Datum = '0000-00-00', 'kein Datum', schritte.Datum) AS Datum, auftrag.Auftragsnummer FROM schritte LEFT JOIN auftrag ON schritte.Auftragsnummer = auftrag.Auftragsnummer LEFT JOIN kunde ON kunde.Kundennummer = auftrag.Kundennummer WHERE auftrag.Rechnungsnummer = 0 AND auftrag.archiviert != 0 AND schritte.istErledigt = 1 ORDER BY schritte.Priority DESC";
