...
--
-- Temporary table structure for view `auftragssumme`
--
...
--
-- Temporary table structure for view `auftragssumme_view`
--
...
--
-- Temporary table structure for view `postendata`
--
...
--
-- Final view structure for view `auftragssumme`
--
...
/*!50013 DEFINER=`auftragsmanager`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `auftragssumme` AS select round(sum(`auftragssumme_view`.`price`),2) AS `orderPrice`,`auftragssumme_view`.`id` AS `id`,`auftrag`.`Datum` AS `Datum`,`auftrag`.`Fertigstellung` AS `Fertigstellung` from (`auftragssumme_view` join `auftrag`) where `auftrag`.`Auftragsnummer` = `auftragssumme_view`.`id` group by `auftragssumme_view`.`id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `auftragssumme_view`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`auftragsmanager`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `auftragssumme_view` AS select `zeit`.`ZeitInMinuten` / 60 * `zeit`.`Stundenlohn` AS `price`,`posten`.`Auftragsnummer` AS `id` from (`zeit` join `posten`) where `zeit`.`Postennummer` = `posten`.`Postennummer` union all select `leistung_posten`.`SpeziefischerPreis` AS `price`,`posten`.`Auftragsnummer` AS `id` from (`leistung_posten` join `posten`) where `leistung_posten`.`Postennummer` = `posten`.`Postennummer` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `postendata`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`auftragsmanager`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `postendata` AS select `posten`.`Postennummer` AS `Postennummer`,`posten`.`Auftragsnummer` AS `Auftragsnummer`,`posten`.`position` AS `position`,`posten`.`angebotsNr` AS `angebotsNr`,`posten`.`rechnungsNr` AS `rechnungsNr`,`posten`.`Posten` AS `Posten`,`posten`.`istStandard` AS `istStandard`,`posten`.`ohneBerechnung` AS `ohneBerechnung`,`posten`.`discount` AS `discount`,`posten`.`isInvoice` AS `isInvoice`,`zeit`.`ZeitInMinuten` AS `ZeitInMinuten`,concat(coalesce(`zeit`.`Beschreibung`,''),coalesce(`leistung_posten`.`Beschreibung`,''),coalesce(`produkt_posten`.`Produktnummer`,'')) AS `Beschreibung` from (((`posten` left join `zeit` on(`posten`.`Postennummer` = `zeit`.`Postennummer`)) left join `leistung_posten` on(`posten`.`Postennummer` = `leistung_posten`.`Postennummer`)) left join `produkt_posten` on(`posten`.`Postennummer` = `produkt_posten`.`Postennummer`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2025-05-30 11:51:10
